...
// TestAdminUser returns a reusable admin user for tests.
func TestAdminUser() *storage.User {
...
func InjectTestAdmin(req *http.Request) *http.Request {
    ctx := context.WithValue(req.Context(), userContextKey, TestAdminUser())
...
func WrapWithAdmin(h http.HandlerFunc) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ctx := context.WithValue(r.Context(), userContextKey, TestAdminUser())
