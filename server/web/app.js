// PrintMaster Server - Web UI JavaScript// Global settings state

// Minimal UI for server management dashboardlet globalSettings = {

    security: {

// ====== Initialization ======        credentials_enabled: true // Default: enabled
...
document.addEventListener('DOMContentLoaded', function() {    }
    ...
    // Initialize theme toggle// Toast notification system

    initThemeToggle();function showToast(message, type = 'success', duration = 3000) {

        const container = document.getElementById('toast_container');

    // Initialize tabs    if (!container) return;

    initTabs();    

        const toast = document.createElement('div');

    // Load initial data    toast.className = `toast toast-${type}`;

    loadServerStatus();    

    loadAgents();    const icons = {

            success: '✓',

    // Set up periodic refresh        error: '✗',

    setInterval(loadServerStatus, 30000); // Every 30 seconds        info: 'ℹ'
...
// ====== Theme Toggle ======        <span class="toast-icon">${icons[type] || icons.info}</span>

function initThemeToggle() {        <span class="toast-message">${message}</span>
    ...
    if (savedTheme === 'light') {    
        ...
        toggle.checked = true;    // Auto-remove after duration

        document.body.classList.add('light-theme');    setTimeout(() => {

    }        toast.classList.add('toast-hiding');

            setTimeout(() => {

    toggle.addEventListener('change', function() {            if (toast.parentNode) {

        if (this.checked) {                toast.parentNode.removeChild(toast);

            document.body.classList.add('light-theme');            }

            localStorage.setItem('theme', 'light');        }, 300); // Match animation duration
...
// ====== Tab Management ======        const titleEl = document.getElementById('confirm_modal_title');

function initTabs() {        const messageEl = document.getElementById('confirm_modal_message');

    const tabs = document.querySelectorAll('.tab');        const confirmBtn = document.getElementById('confirm_modal_confirm');

    const mobileTabs = document.querySelectorAll('.mobile-nav .tab');        const cancelBtn = document.getElementById('confirm_modal_cancel');

    const hamburger = document.querySelector('.hamburger-icon');        const closeX = document.getElementById('confirm_modal_close_x');

    const mobileNav = document.getElementById('mobile_nav');        

            if (!modal || !titleEl || !messageEl || !confirmBtn || !cancelBtn) {

    // Desktop tabs            // Fallback to browser confirm if modal not available

    tabs.forEach(tab => {            resolve(confirm(message));

        tab.addEventListener('click', () => switchTab(tab.dataset.target));            return;

    });        }

            

    // Mobile tabs        // Set content

    mobileTabs.forEach(tab => {        titleEl.textContent = title;

        tab.addEventListener('click', () => {        messageEl.textContent = message;

            switchTab(tab.dataset.target);        

            mobileNav.classList.remove('active');        // Style confirm button based on danger level
    ...
    if (hamburger) {        // Show modal

        hamburger.addEventListener('click', () => {        modal.style.display = 'flex';
...
function switchTab(targetTab) {        };
    ...
    document.querySelectorAll('[data-tab]').forEach(tab => {        // Handle cancel

        tab.classList.add('hidden');        const onCancel = () => {
    ...
    document.querySelectorAll('.tab').forEach(tab => {        
        ...
        tab.classList.remove('active');        // Cleanup function

    });        const cleanup = () => {

                modal.style.display = 'none';

    // Show target tab            confirmBtn.removeEventListener('click', onConfirm);

    const target = document.querySelector(`[data-tab="${targetTab}"]`);            cancelBtn.removeEventListener('click', onCancel);

    if (target) {            if (closeX) closeX.removeEventListener('click', onCancel);

        target.classList.remove('hidden');            modal.removeEventListener('click', onBackdropClick);

    }        };

            

    // Add active class to clicked tab buttons        // Backdrop click closes modal

    document.querySelectorAll(`.tab[data-target="${targetTab}"]`).forEach(tab => {        const onBackdropClick = (e) => {

        tab.classList.add('active');            if (e.target === modal) {
    ...
    if (label) {        // Attach event listeners

        label.textContent = `Menu - ${targetTab.charAt(0).toUpperCase() + targetTab.slice(1)}`;        confirmBtn.addEventListener('click', onConfirm);
...
async function apiCall(endpoint, options = {}) {}
    ...
    try {
        ...
        const response = await fetch(endpoint, {// Check for database rotation warning on page load

            ...options,async function checkDatabaseRotationWarning() {

            headers: {    try {

                'Content-Type': 'application/json',        const response = await fetch('/database/rotation_warning');

                ...options.headers        if (!response.ok) return;
        ...
        return await response.json();                'Database Rotation Notice',

    } catch (error) {                false // Not dangerous, just informational

        console.error('API call failed:', endpoint, error);            );

        showToast('API Error: ' + error.message, 'error');            

        throw error;            if (confirmed) {
...
// ====== Server Status ======                    headers: { 'Content-Type': 'application/json' }
...
async function loadServerStatus() {                });
    ...
    try {            }
        ...
        const health = await apiCall('/health');    } catch (err) {

                console.error('Failed to check rotation warning:', err);

        const statusEl = document.getElementById('server_status');        // Non-fatal - don't interrupt page load
        ...
        if (statusEl) {    }
            ...
            statusEl.innerHTML = `}
                ...
                v${version.version} | // Run a one-off scan using the current ranges textarea (without saving).

                Protocol v${version.protocol_version}// Posts JSON to /scan so the server will use the new pipeline (liveness->detection->deep-scan).

            `;// client-side state for device table sorting

        }let deviceSortField = 'IP';

    } catch (error) {let deviceSortDir = 1; // 1 asc, -1 desc

        const statusEl = document.getElementById('server_status');

        if (statusEl) {function setDeviceSort(field) {

            statusEl.innerHTML = '<span style="color:var(--error);">● Error loading status</span>';    if (deviceSortField === field) deviceSortDir = -deviceSortDir; else { deviceSortField = field; deviceSortDir = 1; }
...
// ====== Agents Management ======    fetch('/devices/discovered').then(r => r.json()).then(d => {

async function loadAgents() {        if (!Array.isArray(d) || d.length === 0) { console.log('No devices to export'); return; }

    try {        const search = (document.getElementById('device_search') || {}).value || '';

        // TODO: Implement /api/v1/agents/list endpoint on server        const rows = [];

        const agents = []; // Placeholder        const headers = ['IP', 'Hostname', 'Manufacturer', 'Model', 'Serial', 'Subnet', 'Gateway', 'PageCount', 'Mono', 'Color'];

                rows.push(headers.join(','));

        renderAgents(agents);        d.forEach(p => {

    } catch (error) {            const combined = [p.ip, p.hostname, p.manufacturer, p.model, p.serial, p.subnet_mask, p.gateway, p.page_count || '', p.mono_impressions || p.total_mono_impressions || '', p.color_impressions || ''];

        document.getElementById('agents_list').innerHTML =             const line = combined.map(v => '"' + String(v || '').replace(/"/g, '""') + '"').join(',');

            '<div style="color:var(--error);">Failed to load agents</div>';            if (!search || JSON.stringify(combined).toLowerCase().includes(search.toLowerCase())) rows.push(line);
...
function renderAgents(agents) {        const a = document.createElement('a'); a.href = url; a.download = 'devices.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);

    const container = document.getElementById('agents_list');    }).catch(e => { console.error('Export failed:', e); showToast('Export failed: ' + e.message, 'error'); });

    const statsContainer = document.getElementById('agents_stats');}

    

    if (!agents || agents.length === 0) {// Render mini vertical toner bars (compact color-coded display)

        container.innerHTML = '<div style="color:var(--muted);">No agents connected yet</div>';function renderMiniTonerBars(tonerLevels) {

        statsContainer.innerHTML = '<div style="color:var(--muted);">Total Agents: 0</div>';    if (!tonerLevels || Object.keys(tonerLevels).length === 0) return '<span style="color:var(--muted)">—</span>';

        return;    let html = '<div style="display:flex;gap:3px;align-items:flex-end;height:24px">';

    }    for (let k in tonerLevels) {

            const v = Number(tonerLevels[k]);

    // Update stats        const pct = isNaN(v) ? 0 : Math.max(0, Math.min(100, v));

    const online = agents.filter(a => a.status === 'online').length;        const keyLower = k.toLowerCase();

    statsContainer.innerHTML = `        let color = '#888'; // default gray for unknown types

        <div><strong>Total Agents:</strong> ${agents.length}</div>        if (keyLower.includes('black') || keyLower === 'k') color = '#111';

        <div><strong>Online:</strong> <span style="color:var(--success)">${online}</span></div>        else if (keyLower.includes('cyan') || keyLower === 'c') color = '#0097a7';

        <div><strong>Offline:</strong> <span style="color:var(--muted)">${agents.length - online}</span></div>        else if (keyLower.includes('magenta') || keyLower === 'm') color = '#c2185b';

    `;        else if (keyLower.includes('yellow') || keyLower === 'y') color = '#fbc02d';

            const height = Math.max(2, (pct * 20) / 100); // 20px max height

    // Render agent cards        html += '<div title="' + k + ': ' + pct + '%" style="width:8px;height:' + height + 'px;background:' + color + ';border-radius:2px 2px 0 0;border:1px solid rgba(0,0,0,0.3)"></div>';

    container.innerHTML = agents.map(agent => `    }

        <div class="panel" style="margin-bottom:12px;">    html += '</div>';

            <div style="display:flex;justify-content:space-between;align-items:center;">    return html;

                <div>}

                    <strong>${agent.name || agent.agent_id}</strong>

                    <div style="color:var(--muted);font-size:13px;">${agent.hostname || 'Unknown hostname'}</div>// Filter table rows by search text

                </div>function filterTable(tableId, searchText) {

                <div style="text-align:right;">    const container = document.getElementById(tableId);

                    <div style="color:${agent.status === 'online' ? 'var(--success)' : 'var(--muted)'}">    if (!container) return;

                        ${agent.status === 'online' ? '● Online' : '○ Offline'}

                    </div>    const table = container.querySelector('table');

                    <div style="color:var(--muted);font-size:12px;">    if (!table) return;

                        Last seen: ${agent.last_heartbeat ? formatTimeAgo(agent.last_heartbeat) : 'Never'}

                    </div>    const tbody = table.querySelector('tbody');

                </div>    if (!tbody) return;

            </div>

        </div>    const searchLower = searchText.toLowerCase().trim();

    `).join('');    const rows = tbody.querySelectorAll('tr');

}

    rows.forEach(row => {

// ====== Devices Management ======        if (!searchLower) {

async function loadDevices() {            row.style.display = '';

    try {            return;

        // TODO: Implement /api/v1/devices/list endpoint on server        }

        const devices = []; // Placeholder

                const text = row.textContent.toLowerCase();

        renderDevices(devices);        row.style.display = text.includes(searchLower) ? '' : 'none';
...
function renderDevices(devices) {async function copyToClipboard(text, iconElement, customMessage) {

    const container = document.getElementById('devices_cards');    if (!text || text === 'N/A' || text === 'Not Set') return;

    const statsContainer = document.getElementById('devices_stats');    

        try {

    if (!devices || devices.length === 0) {        if (navigator.clipboard && window.isSecureContext) {

        container.innerHTML = '<div style="color:var(--muted);">No devices found</div>';            await navigator.clipboard.writeText(text);

        statsContainer.innerHTML = '<div style="color:var(--muted);">Total Devices: 0</div>';        } else {

        return;            // Fallback for non-secure contexts

    }            const ta = document.createElement('textarea');

                ta.value = text;

    // Update stats            ta.setAttribute('readonly', '');

    statsContainer.innerHTML = `            ta.style.position = 'fixed';

        <div><strong>Total Devices:</strong> ${devices.length}</div>            ta.style.left = '-9999px';

    `;            document.body.appendChild(ta);

                ta.select();

    // Render device cards (simplified)            document.execCommand('copy');

    container.innerHTML = devices.map(device => `            ta.remove();

        <div class="panel" style="margin-bottom:12px;">        }

            <div><strong>${device.manufacturer} ${device.model}</strong></div>        

            <div style="color:var(--muted);font-size:13px;">${device.ip}</div>        // Visual feedback on the icon if provided

            <div style="color:var(--muted);font-size:12px;">Serial: ${device.serial || 'N/A'}</div>        if (iconElement) {

        </div>            iconElement.classList.add('copied');
...
function formatTimeAgo(timestamp) {        
    ...
    const date = new Date(timestamp);        // Show toast notification

    const now = new Date();        if (customMessage) {

    const seconds = Math.floor((now - date) / 1000);            showToast(customMessage, 'success', 2000);

            } else {

    if (seconds < 60) return 'Just now';            const displayText = text.length > 40 ? text.substring(0, 40) + '...' : text;

    if (seconds < 3600) return Math.floor(seconds / 60) + ' minutes ago';            showToast(`Copied: ${displayText}`, 'success', 2000);

    if (seconds < 86400) return Math.floor(seconds / 3600) + ' hours ago';        }

    return Math.floor(seconds / 86400) + ' days ago';    } catch (err) {

}        console.error('Copy failed:', err);

        showToast('Failed to copy to clipboard', 'error');

function showToast(message, type = 'info') {        throw err;
    ...
    const toast = document.createElement('div');// Get status indicator based on toner levels

    toast.className = `toast toast-${type}`;function getStatusIndicator(toners) {

    toast.textContent = message;    if (!toners || typeof toners !== 'object') {
    ...
    setTimeout(() => toast.classList.add('show'), 10);    let minLevel = 100;

    setTimeout(() => {    for (const color in toners) {

        toast.classList.remove('show');        const level = toners[color] || 0;

        setTimeout(() => toast.remove(), 300);        if (level < minLevel) minLevel = level;
...
// ====== Settings Management ======        return '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#dc322f;margin-right:6px" title="Critical: ' + minLevel + '% toner"></span>';

const settingsState = {    } else if (minLevel <= 20) {

    autosave: false,        return '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#b58900;margin-right:6px" title="Low: ' + minLevel + '% toner"></span>';

    modified: false,    } else {

    current: {},        return '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#859900;margin-right:6px" title="Good: ' + minLevel + '% toner"></span>';
...
function initSettings() {// Sort table by column

    const autosaveCheckbox = document.getElementById('settings_autosave');function sortTable(tableId, columnIndex, dataType = 'string') {

    if (autosaveCheckbox) {    const container = document.getElementById(tableId);

        autosaveCheckbox.addEventListener('change', () => {    if (!container) return;

            settingsState.autosave = autosaveCheckbox.checked;

            if (settingsState.autosave && settingsState.modified) {    const table = container.querySelector('table');

                saveSettings();    if (!table) return;
    ...
    // Load settings from server    const rows = Array.from(tbody.querySelectorAll('tr'));
...
async function loadSettings() {    const currentDir = tbody.getAttribute('data-sort-dir') || 'asc';

    // TODO: Implement settings endpoint    const newDir = (currentSort == columnIndex && currentDir === 'asc') ? 'desc' : 'asc';
...
async function saveSettings() {        const bText = b.cells[columnIndex]?.textContent.trim() || '';

    // TODO: Implement settings save endpoint

    console.log('Saving settings...');        let aVal = aText;

    showToast('Settings saved', 'success');        let bVal = bText;

}

        if (dataType === 'number') {

// ====== Log Viewer ======            aVal = parseFloat(aText.replace(/[^0-9.-]/g, '')) || 0;

function initLogViewer() {            bVal = parseFloat(bText.replace(/[^0-9.-]/g, '')) || 0;

    // TODO: Implement server log streaming        } else if (dataType === 'ip') {

    const logEl = document.getElementById('log');            aVal = aText.split('.').map(n => parseInt(n) || 0);

    if (logEl) {            bVal = bText.split('.').map(n => parseInt(n) || 0);

        logEl.textContent = 'Server log viewer - coming soon\nConnect to server API for live logs...';            for (let i = 0; i < 4; i++) {
...
// Initialize settings and logs when DOM loads        }

if (document.readyState === 'loading') {

    document.addEventListener('DOMContentLoaded', () => {        if (dataType === 'number') {

        initSettings();            return newDir === 'asc' ? aVal - bVal : bVal - aVal;

        initLogViewer();        }

    });

} else {        return newDir === 'asc' ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
