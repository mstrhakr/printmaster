...
- Add SSO (OIDC first, SAML deferred) plus robust local login as authentication options.
...
3) Auth: OIDC + local login (High priority)
- Why: secure access to tenant admin screens and SSO for customers.
- What: add OIDC provider support plus local username/password fallback; session management; mapping of SSO users to tenants/roles.
- Acceptance: OIDC login + local login flows work; admins can map users to tenants/roles. (SAML remains on the backlog until after GA.)
...
- Auth: use `go-oidc` for OIDC; provide mapping rules (email domain/group -> tenant) and an invite flow for explicit mappings. (SAML via `crewjam/saml` is postponed.)
...
- SAML: deferred until after initial multi-tenant GA; when we pick it back up, evaluate in-process `crewjam/saml` vs. proxy integration.
